...
BASE_PATH = r"C:\Users\mohit\Desktop\Recordings"

def create_dirs():
    os.makedirs(os.path.join(BASE_PATH, "Screen"), exist_ok=True)
    os.makedirs(os.path.join(BASE_PATH, "Audio"), exist_ok=True)
    os.makedirs(os.path.join(BASE_PATH, "ScreenRecorded"), exist_ok=True)
...
def Record_Option(option):
    ...
    VideoFile_name = os.path.join(BASE_PATH, "Screen", f"VideoFile-{time_stamp}.mp4")
    AudioFile_name = os.path.join(BASE_PATH, "Audio", f"AudioFile-{time_stamp}.wav")
    OutputFileName = os.path.join(BASE_PATH, "ScreenRecorded", f"VideoFile-{time_stamp}.mp4")
...
def VoiceRecording(stream, frames, AudioFile_name, audio):
    print("Voice Recording Started! Press 'q' to stop.")
